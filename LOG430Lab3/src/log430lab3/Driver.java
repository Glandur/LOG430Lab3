...
public class Driver {
	...
	public DeliveryList getDeliveriesAssigned() {
        ...
        private int getDriverAssignedTime()
        {
            boolean done;
            Delivery delivery;
            int assignedTime = 0;
            this.getDeliveriesMadeList().goToFrontOfList();
            done = false;

            while (!done) {

                delivery = this.getDeliveriesMadeList().getNextDelivery();

                if (delivery == null) {
                    done = true;
                } else {
                    assignedTime+=delivery.getEstimatedDeliveryTimeInMinutes();
                } // if
            } // while
        ...
        public boolean canAddDelivery(Delivery delevery)
        {
            int maximumTime = 0 ;
            if (this.getType().equals("JNR"))
                maximumTime = 12*60;
            else if (this.getType().equals("SNR"))
                maximumTime = 8*60;
            
            return maximumTime >= getDriverAssignedTime() + delevery.getEstimatedDeliveryTimeInMinutes();    
        }
