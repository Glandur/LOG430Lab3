		...
		if (registrationNumber.compareTo((Integer)notificationNumber) == 0) {
			...
			if (myDriver != null) {
				...
				myDelivery = menu.pickDelivery(CommonData.theListOfDeliveries.getListOfDeliveries());
                                ...
                                boolean deliveryConflict = false;
                                while((currentDelivery = myDriver.getDeliveriesAssigned().getNextDelivery()) != null)
                                {
                                    if (currentDelivery.isConflictWith(myDelivery)) {
                                        deliveryConflict = true;
                                        break;
                                    }
                                }
                                
                                myDelivery.getDriversAssigned().goToFrontOfList();
                                if(myDelivery.getDriversAssigned().getNextDriver() != null)
                                {
                                    System.out.println("\n\n *** Delivery already assigned ***");
                                }
                                else if(myDriver.canAddDelivery(myDelivery))
                                {
                                    System.out.println("\n\n *** Driver has reached his maximum deliveries ***");
                                }
                                else if (deliveryConflict) 
                                {
                                    System.out.println("\n\n *** Delivery in conflict with one of the driver's assigned deliveries ***");
                                }
                                else if (myDelivery != null)	{	
					/*
